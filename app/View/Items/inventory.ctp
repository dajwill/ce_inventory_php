<div class="iinventory index">
	<h2><?php echo __('All Items'); ?></h2>
	...
	<tr>
		...
		<td><?php echo h($item['Item']['model_number']); ?>&nbsp;</td>
		<td>
			<?php
				if ($item['Item']['checked_out']):
					echo h($check_outs->findByItemId($item['Item']['id'])['User']['email']);
				endif;
			?>
			&nbsp;
		</td>
		<td class="actions">
			...
			<?php
				if (!$item['Item']['checked_out']):
					echo $this->Html->link(__('Check Out'), array('controller' => 'check_outs', 'action' => 'add', $item['Item']['id']));
				endif;
			?>
